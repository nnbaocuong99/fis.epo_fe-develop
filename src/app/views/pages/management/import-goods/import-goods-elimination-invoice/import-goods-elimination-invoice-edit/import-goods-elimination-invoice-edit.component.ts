...
@Component({
  selector: 'app-import-goods-elimination-invoice-edit',
  templateUrl: './import-goods-elimination-invoice-edit.component.html',
  styleUrls: ['./import-goods-elimination-invoice-edit.component.scss']
})
export class ImportGoodsEliminationInvoiceEditComponent extends BaseFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.dialogRef.input.rowData) {
      ...
      const initSub = forkJoin([
        this.purchaseInvoiceService.selectById(this.dialogRef.input.rowData.piId),
      ]).subscribe(res => {
        if (res) {
  ...
  public getEliminationInfoView(piId: string): void {
    ...
    requestIgElimination.piId = piId;
